    ...
    <div class="ant-row ant-row-center">
        <div class="ant-col ant-col-xs-24 ant-col-md-16">
            <table id="history-table" class="table table-borderless table-striped table-earning" style="width:100%">
                ...
                <tbody>
                    @foreach($data["data"] as $key => $history)
                    <tr>
                        <td align="center" hidden>{{ $history["CreatedAt"] }}</td>
                        <td align="center">{{ date('d-m-Y H:i:s',strtotime($history["CreatedAt"])) }}</td>
                        <td align="center">{{ $history["Name"] }}</td>
                        <td align="center">{{ number_format(floor($history["Point"] * 100) / 100,2) }}</td>
                        <td align="center">{{ date('d-m-Y H:i:s',strtotime($history["ExpireDate"])) }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
